// src/settings/setting_other_lesson5-16.tsx
...
// Helper: แปลงข้อความธรรมดา ให้กลายเป็น Array ของ Tokens (อักษร + พินอิน) อัตโนมัติ
...
export default function SettingOtherLesson5_16({ section, cardId, lessonId, updateSectionState }: Props) {
  
  // โหลดข้อมูลเริ่มต้น (Default Data ตามรูปภาพเป๊ะๆ)
  useEffect(() => {
    if (!section.cards || section.cards.length === 0) {
      ...
      updateSectionState(cardId, lessonId, section.id, (s) => ({
        ...
        cards: s.cards || [
          createDefaultCard('给哥哥一个杯子。', '杯子', ''),
          createDefaultCard('给爷爷一个勺子。', '勺子', ''),
          createDefaultCard('给姐姐一杯可乐。', '杯', ''),
          createDefaultCard('给爸爸一块面包。', '块', ''),
          createDefaultCard('给妈妈一双筷子。', '双', ''),
          createDefaultCard('给我一个叉子。', '个', '')
        ]
      }));
    }
  }, []);
  ...
  // รับข้อความใหม่ แล้วแปลงเป็น Tokens ทันที (Auto Pinyin)
  ...
  // สลับสถานะ isBlank เมื่อคุณครูคลิกที่คำ
  ...
  return (
    <div className="bg-emerald-50/50 p-4 rounded-lg border border-emerald-100 font-sans mt-4 relative">
      <h4 className="font-bold text-emerald-800 mb-4 text-sm uppercase tracking-wide border-b border-emerald-200 pb-2 flex items-center gap-2">
        <Edit3 size={18} /> แบบเรียน: เติมคำพินอินในช่องว่าง (Other_lesson5-16)
      </h4>
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200">
        <div className="flex items-center justify-between border-b pb-2 mb-4">
          <label className="text-xs font-bold text-slate-600 uppercase flex items-center gap-2"><Sparkles size={14} className="text-emerald-500"/> ระบบสร้างช่องว่างอัจฉริยะ</label>
        ...
        <div className="grid grid-cols-1 gap-6">
            ...
            return (
              <div key={card.id} className="flex flex-col gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm relative">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-[11px] font-bold text-emerald-700 bg-emerald-100 px-3 py-1 rounded-full">ข้อที่ {idx + 1}</div>
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1 space-y-3">
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 mb-1 block uppercase">1. พิมพ์ประโยคเต็มที่นี่ (ระบบจะแยกคำให้)</label>
                      <input 
                        type="text" 
                        value={rawSentence} 
                        onChange={(e) => handleUpdateSentence(idx, e.target.value)} 
                        className="w-full px-3 py-2 text-sm border border-emerald-200 rounded-lg focus:ring-2 focus:ring-emerald-400 font-serif" 
                    ...
                    <div className="bg-white p-3 rounded-lg border border-slate-100">
                      <label className="text-[10px] font-bold text-orange-500 mb-2 block uppercase animate-pulse">
                        2. กดที่คำที่ต้องการให้ "พินอิน" หายไป (พินอินของคำนั้นจะถูกดึงไปเป็นตัวเลือก A, B, C อัตโนมัติ)
                      </label>
                  ...
                  <div className="w-full md:w-64 shrink-0">
                    ...
                    <input 
                      ...
                      placeholder="วางลิ้งค์รูปภาพ (Google Drive)..."
                    />
                    {card.imageUrl && (
          ...
          <button onClick={handleAddCard} className="w-full py-4 flex items-center justify-center gap-2 border-2 border-dashed border-emerald-200 text-emerald-600 rounded-xl text-sm font-bold hover:bg-emerald-50 transition-all">
