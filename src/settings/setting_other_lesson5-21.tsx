// src/settings/setting_other_lesson5-21.tsx
...
export default function SettingOtherLesson5_21({ section, cardId, lessonId, updateSectionState }: Props) {
  ...
  useEffect(() => {
    if (!section.cards || section.cards.length === 0) {
      ...
      updateSectionState(cardId, lessonId, section.id, (s) => ({
        ...
        mainTitle: s.mainTitle || '5. 看图片，选择正确的汉字。',
        subTitle: s.subTitle || 'ดูภาพแล้วเลือกตัวอักษรจีนที่ถูกต้อง',
        cards: s.cards || [
          createDefaultCard('给我一杯牛奶，谢谢！', '杯', ''),
          createDefaultCard('娜林给王冬冬一双筷子。', '双', ''),
          createDefaultCard('爸爸给妹妹一双鞋。', '双', ''),
          createDefaultCard('给我一个叉子，谢谢！', '个', ''),
          createDefaultCard('给妈妈一个勺子。', '个', ''),
          createDefaultCard('弟弟给我一块蛋糕。', '块', '')
        ]
      }));
    }
  }, []);
  ...
  return (
    <div className="bg-orange-50/50 p-4 rounded-lg border border-orange-100 font-sans mt-4 relative">
      <h4 className="font-bold text-orange-800 mb-4 text-sm uppercase tracking-wide border-b border-orange-200 pb-2 flex items-center gap-2">
        <Edit3 size={18} /> แบบเรียน: เติมอักษรจีนในช่องว่าง (Other_lesson5-21)
      </h4>
            ...
            return (
              <div key={card.id} className="flex flex-col gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm relative">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-[11px] font-bold text-orange-700 bg-orange-100 px-3 py-1 rounded-full">ข้อที่ {idx + 1}</div>
