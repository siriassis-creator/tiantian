...
export interface PatternNoteData {
...
interface Props {
  data: PatternNoteData;
  onUpdateNote?: (newNote: string) => void;
}

export default function PatternNote({ data, onUpdateNote }: Props) {
  const [isNoteOpen, setIsNoteOpen] = useState(false);
  const [tempNote, setTempNote] = useState(data.teacherNote || '');

  // ฟังก์ชันกดอ่านออกเสียง (อ่านเฉพาะภาษาจีน)
  const speakChinese = (text: string) => {
    if (!text) return;

    // 1. ตัดข้อความที่อยู่ในวงเล็บออกทั้งหมด เช่น (zhè, this) หรือ ( 1 )
    let textToSpeak = text.replace(/[\(（][^\)）]*[\)）]/g, '');

    // 2. ตัดตัวอักษรภาษาอังกฤษ และสระพินอินออกให้หมด เผื่อพิมพ์ต่อท้ายไว้
    textToSpeak = textToSpeak.replace(/[a-zA-Zāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜü]/g, '');

    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      // ส่งเฉพาะข้อความที่ถูกกรองแล้วไปให้อ่าน
  ...
  return (
    <div className="flex w-full gap-2 transition-all duration-500 items-start my-4 font-sans">
      <div className="bg-white/90 p-8 md:p-12 rounded-xl shadow-sm border border-slate-100 flex-1 relative overflow-hidden z-[1]">
        ...
        {/* --- ส่วนตัวอย่างประโยค (รันตัวเลขให้อัตโนมัติ พร้อมออกเสียงเฉพาะจีน) --- */}
          ...
          <div className="px-8 mt-8 space-y-4">
            {data.examples.map((example, index) => (
              <div
                ...
                className="flex items-start text-[22px] font-serif text-slate-800 leading-relaxed cursor-pointer group hover:text-indigo-600 transition-colors"
                title="คลิกเพื่อฟังเสียงอ่านภาษาจีน"
              >
                <span className="mr-3 text-slate-400 shrink-0 font-sans group-hover:text-indigo-400 transition-colors">
                  （{index + 1}）
                </span>
