...
export interface PatternSentence4ColsData {
...
interface Props {
  data: PatternSentence4ColsData;
  onUpdateNote?: (newNote: string) => void;
}

export default function PatternSentence4Cols({ data, onUpdateNote }: Props) {
  const [isNoteOpen, setIsNoteOpen] = useState(false);
  const [tempNote, setTempNote] = useState(data.teacherNote || '');

  // ฟังก์ชันกดอ่านออกเสียงทั้งประโยค
    ...
    <div className="flex w-full gap-2 transition-all duration-500 items-start my-4 font-sans">
      <div className="bg-white/90 p-8 md:p-12 rounded-xl shadow-sm border border-slate-100 flex-1 relative overflow-hidden z-[1]">
        ...
        <div className="px-4 mt-8">
          <div className="rounded-xl overflow-hidden border-2 border-slate-200 bg-white shadow-sm">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-slate-50">
                  <th
                    rowSpan={2}
                    className="border border-slate-200 p-4 text-[17px] font-normal text-slate-500 text-center align-middle w-1/4"
                  >
                    {data.headerSubject || 'Subject'}
                  </th>
                  <th
                    colSpan={3}
                    className="border border-slate-200 p-2 text-[17px] font-normal text-slate-500 text-center w-3/4"
                  >
                    {data.headerPredicate || 'Predicate'}
                  </th>
                </tr>
                <tr className="bg-slate-50">
                  <th className="border border-slate-200 p-2 text-[16px] font-normal text-slate-500 text-center w-1/4">
                    {data.headerVerb || 'Verb'}
                  </th>
                  <th className="border border-slate-200 p-2 text-[16px] font-normal text-slate-500 text-center w-1/4">
                    {data.headerObject || 'Noun/Noun Phrase'}
                  </th>
                  <th className="border border-slate-200 p-2 text-[16px] font-normal text-slate-500 text-center w-1/4">
                    {data.headerParticle || '吗?'}
                  </th>
                </tr>
              </thead>
              <tbody>
                {(data.tableRows || []).map((row, index) => (
                  ...
                  >
                    ...
                    <td className="border border-slate-200 p-4 text-[26px] font-serif text-slate-700 text-center text-indigo-600">
                      {row.col4}
                    </td>
                  </tr>
                ))}
